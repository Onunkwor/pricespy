...
export async function scrapeAmazonProduct(productUrl: string, maxRetries = 3) {
  if (!productUrl) {
  ...
  while (retryCount < maxRetries) {
    try {
      ...
      const priceArr = $(
        "#corePriceDisplay_desktop_feature_div div.a-section span"
      )
        .text()
        .trim()
        .split(/\s{4,}/)
      ...
      const originalPrice = (currentPrice / (1 - discountRate / 100)).toFixed(
        2
      );
      const outOfStock =
